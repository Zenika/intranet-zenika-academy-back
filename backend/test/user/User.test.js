...
describe("Test User Controller Api call", () => {
  ...
  beforeAll(async () => {
    await Users.create({...defaultUser, password: await bcrypt.hash("user", 10)});
  ...
  it("Create a new user with admin role", async done => {
    ...
    const response = await httpMethod.post("/api/users", token, body).expect(201);
  ...
  it("Delete the created student user", async done => {
    await httpMethod.delete(`/api/users/${testStudentUserId}`, token).expect(200);
    done();
  });
});
